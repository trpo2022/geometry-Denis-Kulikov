...
CTEST(input_arg, figur_1)
{
    ...
    if (circle_test.point[0] == expected.point[0]
        && circle_test.point[1] == expected.point[1]
        && circle_test.radius == expected.radius)
        result = 1;
    else
        result = 0;
...
CTEST(fun_area_perimetr, geometry_1)
{
    ...
    if (info_test.perimeter == expected.perimeter
        && info_test.area == expected.area)
        result = 1;
    else
        result = 0;
